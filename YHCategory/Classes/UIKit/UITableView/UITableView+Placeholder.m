...
+ (void)load {
    ...
    for (NSUInteger index = 0; index < sizeof(selectors) / sizeof(SEL); ++index) {
        ...
        SEL swizzledSelector = NSSelectorFromString([@"yh_" stringByAppendingString:NSStringFromSelector(originalSelector)]);
...
- (void)yh_insertSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation {
    [self showPlaceholderAction];
    [self yh_insertSections:sections withRowAnimation:animation];
}

- (void)yh_deleteSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation {
    [self showPlaceholderAction];
    [self yh_deleteSections:sections withRowAnimation:animation];
}

- (void)yh_reloadSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation {
    [self showPlaceholderAction];
    [self yh_reloadSections:sections withRowAnimation:animation];
}

- (void)yh_insertRowsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation {
    [self showPlaceholderAction];
    [self yh_insertRowsAtIndexPaths:indexPaths withRowAnimation:animation];
}

- (void)yh_deleteRowsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation {
    [self showPlaceholderAction];
    [self yh_deleteRowsAtIndexPaths:indexPaths withRowAnimation:animation];
}

- (void)yh_reloadRowsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation {
    [self showPlaceholderAction];
    [self yh_reloadRowsAtIndexPaths:indexPaths withRowAnimation:animation];
}
...
- (UIView *)defaultNoDataView {
    ...
    self.noDataView = ({
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(yh_tapDefalutNoDataView:)];
...
- (void)yh_tapDefalutNoDataView:(UITapGestureRecognizer *)tap {
